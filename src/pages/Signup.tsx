...
import { signUpWithEmail, signInWithGoogle, signInWithApple, UserRole } from '../lib/supabase';
...
const Signup: React.FC = () => {
  ...
  const handleAppleSignup = async () => {
    try {
      ...
      if (error) {
        toast.error(error.message);
        console.error('Apple signup error details:', error);
      }
    } catch (error) {
      console.error('Unexpected Apple signup error:', error);
      toast.error('An error occurred with Apple signup');
    }
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-secondary-800 p-8 rounded-2xl shadow-soft">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input id="password" name="password" type="password" autoComplete="new-password" required value={password} onChange={e => setPassword(e.target.value)} className="appearance-none block w-full px-3 py-3 pl-10 border border-secondary-300 dark:border-secondary-600 rounded-xl shadow-sm placeholder-secondary-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-secondary-700 text-secondary-900 dark:text-secondary-100" placeholder="••••••••" />
              </div>
            </div>
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <select id="role" name="role" value={role} onChange={e => setRole(e.target.value as UserRole)} className="appearance-none block w-full px-3 py-3 pl-10 border border-secondary-300 dark:border-secondary-600 rounded-xl shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-secondary-700 text-secondary-900 dark:text-secondary-100">
          ...
          <div>
            <button type="submit" disabled={isLoading} className="group relative w-full flex justify-center py-3 px-4 border border-transparent rounded-xl text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed">
              {isLoading ? <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                  ...
                  <UserPlus size={18} className="text-primary-300" />
                </span>}
              Sign up
            </button>
          </div>
        ...
        <div className="mt-6">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            ...
            <button onClick={handleAppleSignup} className="w-full inline-flex justify-center py-3 px-4 border border-secondary-300 dark:border-secondary-600 rounded-xl shadow-sm bg-white dark:bg-secondary-700 text-sm font-medium text-secondary-700 dark:text-secondary-300 hover:bg-secondary-50 dark:hover:bg-secondary-600">
